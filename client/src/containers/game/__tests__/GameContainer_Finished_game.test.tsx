...
vi.mock("@client/api/queries/useGetPlayer", () => ({
  useGetPlayer: ({ player_id }: { player_id: string }) => {
    const player = player_id === PLAYER_1_ID ? player1Mock : currentUserMock
...
describe("GameContainer", () => {
  describe("Finished Game", () => {
    it("Should render Info section", () => {
      ...
      expect(title.textContent).toEqual(finishedGameMock.name)

      expect(title.parentNode?.childNodes[1].textContent).toEqual("Finished")
      expect(title.parentNode?.childNodes[2].textContent).toEqual(
        "This game took 33 moves",
      )
    })
    ...
    it("Should render Players section", () => {
      ...
      expect(tableBodySecondRowCells[1].textContent).toEqual(
        currentUserMock.username,
      )
      expect(tableBodySecondRowCells[2].textContent).toEqual("")
    })
    ...
    it("Should render Status section", () => {
      ...
      expect(title).toBeInTheDocument()
      expect(title.textContent).toEqual("Result")

      const cardBody =
        title.parentNode?.parentElement?.querySelector(".card-body")
      const cardBodyContent = cardBody?.querySelector("div")

      expect(cardBodyContent?.textContent).toEqual(
        `${player1Mock.username} won`,
      )
    })
